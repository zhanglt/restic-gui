...
**主要组件：**
- **MainWindow：** 主窗口，包含侧边栏导航和内容区域
- **WizardDialog系列：** 各种向导界面（仓库创建、备份配置、恢复等）
- **ConfigDialog系列：** 配置对话框
- **ProgressDialog：** 进度显示窗口
- **ListView/TreeView：** 数据展示组件
...
| 异步处理 | QThread + 信号/槽 | Qt标准异步模式，避免阻塞UI |
| JSON解析 | QJsonDocument | Qt内置，无需第三方库 |
| 日志系统 | qDebug + QFile | 简单有效，可自定义输出 |
